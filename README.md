...
- **角色控制**：重構的動畫系統，包含待機 (Idle)、奔跑 (Run)、跳躍 (Jump)、掉落 (Fall)、攻擊 (Attack) 與死亡 (Death)。
- **NPC 系統**：新增 `mask_boy` NPC，具備自動循環播放 Idle/Walk/Attack 動畫的功能 (每 3 秒切換)。
- **資源處理**：包含自動化 PowerShell 腳本 (`generate_all_sheets.ps1`)，可將序列圖檔合併為 Spritesheet。
- **操作優化**：新增空中掉落狀態偵測與單次攻擊邏輯。
- **操作方式**：
  - **移動**：WASD 或 方向鍵
